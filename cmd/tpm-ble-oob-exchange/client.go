...
func main()  {
	pemCert, err := ble.ReadCertificate("hci0", "00:1A:7D:DA:71:07")
	if err != nil {
	...
	err = crypto.VerifyCertificate([]byte(caCertPEM2), pemCert)
	if err != nil {
		log.Fatalf("Could not verify certificate. Error: %s", err)
	}

	log.Println("Certificate is valid (signed by the CA)")
