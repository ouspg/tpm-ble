...
CGO_ENABLED=1 GOARCH=arm go build -o ./build/oura_client examples/oura-ring/client.go

echo "Transferring..."
rpi_scp ./build/oura_client pi@$HOST:/tmp/oura_client
...
ssh -t -i ./keys/ssh/pi_key.priv pi@$HOST 'sudo LD_LIBRARY_PATH="/usr/local/lib" /tmp/oura_client'