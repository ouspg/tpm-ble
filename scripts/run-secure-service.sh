...
CGO_ENABLED=1 GOARCH=arm go build -o ./build/raspi_service examples/secure-gatt/service/main.go

echo "Transferring..."
rpi_scp ./build/raspi_service pi@$HOST:/tmp/raspi_service
...
echo "Run"
ssh -t -i ./keys/ssh/pi_key.priv pi@$HOST 'sudo LD_LIBRARY_PATH="/usr/local/lib" /tmp/raspi_service'